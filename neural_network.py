...
# Neural network layers: 784(input) - 160(hidden) - 10(output)
class NeuralNetwork:
    ...
    def train(self, images_train, labels_train, learn_rate):
        ...
        for training_example_index in indices:
            ...
            term2 = query[2].reshape(1, 160)
            weights2_gradient = term2 * term1

            term1 = query[4] - desired_output
            term2 = np.sum(self.weights2, axis=0).reshape(1, 160)
            term3 = self.reLU_derivative(query[1]).reshape(1, 160)
            term4 = query[0].reshape(1, 1, 784)
            weights1_gradient = (term1 * term2 * term3).reshape(10, 160, 1)
            ...
            biases1_gradient = np.sum(biases1_gradient, axis=0).reshape(160, 1)
    ...
    def query(self, pixels):
        ...
        # Compute the 160 neuron values of the hidden layer 'h'
        unactive_h = self.weights1 * pixels.reshape(1, len(pixels))
        unactive_h = np.sum(unactive_h, axis=1).reshape(160, 1)
        ...
        unactive_o = self.weights2 * active_h.reshape(1, 160)
